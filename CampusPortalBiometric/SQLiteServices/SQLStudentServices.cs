...
namespace CampusPortalBiometric.SQLiteServices
{
    public class SQLStudentServices
    {
        ...
        {
            SQLiteCommand sqlite_cmd = connection.CreateCommand();
            sqlite_cmd.CommandText = "Select * FROM NonRegStudent";
            SQLiteDataReader sqlite_datareader = sqlite_cmd.ExecuteReader();
            List<Student> AllStudents = new List<Student>();
        ...
        public void SaveNonRegisteredStudents(List<Student> Students)
        {
            String query = "INSERT INTO NonRegStudent (Id ,Name, Father_Name, Class,Fingerprint) VALUES (@Id ,@Name, @Father_Name, @Class,@Fingerprint)";
        ...
        {
            DeleteNonReg(student.Id);
            Register(student, XMLPrint);
        }

        private void DeleteNonReg(string Id)
        {
            String query = "DELETE FROM NonRegStudent WHERE Id=@Id";
            using (SQLiteCommand command = new SQLiteCommand(query, connection))
            {
                command.Parameters.AddWithValue("@Id", Id);
                int result = command.ExecuteNonQuery();
        ...
        private void Register(Student student, string XMLPrint)
        {
            String query = "INSERT INTO RegStudent (Id ,Name, Father_Name, Class,Fingerprint) VALUES (@Id ,@Name, @Father_Name, @Class,@Fingerprint)";
